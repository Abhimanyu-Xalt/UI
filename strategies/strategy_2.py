...
# from utils import placeBackBet, placeLayBet

def strategy_2(surface,market_catalogue,SSOID):
    import datetime
    import requests
    import urllib , json
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for market in market_catalogue:
        success_bets_dict = {"Match": '',"strategy": '',"Amount": 0,"Type": '',"Odds": 0, "Status": '', "Player": ''}
        ...
        back_bet_price , lay_bet_price = getBothPrice(market_book, runner1_selectionID)
        # print('Back Bet Price: ', back_bet_price)
        # print('Lay Bet Price: ', lay_bet_price)
        if back_bet_price == None:
        ...
        prob_star = round(1/(back_bet_price+lay_bet_price),2)
        # print('probability star:' , prob_star)
        ...
        if len(final_df) == 0:
            ...
            # print('Player winning Probability:', final_df['Player1_Win_prob (0)'].values[0])
            if final_df['Player1_Win_prob (0)'].values[0] > prob_star:
                BackBet_status,Back_bet_reason,type,odds = placeBackBet(market_id, selection_id, back_bet_price, size)
                if BackBet_status != 'SUCCESS':
                    ...
                    fail_bets.append(fail_bets_dict)        
                if BackBet_status == 'SUCCESS':
                    success_bets_dict['Match'] = match
                    success_bets_dict['strategy'] = strategy_name
                    success_bets_dict['Type'] = type
                    success_bets_dict['Amount'] = size
                    success_bets_dict['Odds'] = odds
                    success_bets_dict['Status'] = BackBet_status
                    success_bets.append(success_bets_dict)
            else:
                LayBet_status,Lay_bet_reason,type,odds = placeLayBet(market_id, selection_id, lay_bet_price, size)
                if LayBet_status == 'SUCCESS':
                    ...
                    success_bets_dict['Amount'] = size
                    success_bets_dict['Odds'] = odds
                    success_bets_dict['Status'] = LayBet_status
                    success_bets.append(success_bets_dict)
                else : 
                    fail_bets_dict["Match"] = match
                    fail_bets_dict["strategy"] = strategy_name
